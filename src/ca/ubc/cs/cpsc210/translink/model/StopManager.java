...
public class StopManager implements Iterable<Stop> {
    ...
    public Stop getStopWithNumber(int number) {
        if (stopMap.containsKey(number)) {
            Stop want = stopMap.get(number);
            return want;
        }else{
            Stop want = new Stop(number, "", new LatLon(-49.2, 123.2));
            stopMap.put(number, want);
            return want;
        }
    }
    ...
    public Stop getStopWithNumber(int number, String name, LatLon locn) {
        if (stopMap.containsKey(number)) {
            ...
            Stop want = new Stop(number, name, locn);
            stopMap.put(number, want);
            return want;
        }
    }
    ...
    public Stop findNearestTo(LatLon pt) {
        Stop nearestpt = null;
        double shortest = RADIUS + 1;
        Iterator<Stop> it = this.iterator();
        while (it.hasNext()){
            Stop next = it.next();
            if (SphericalGeometry.distanceBetween(next.getLocn(), pt) < RADIUS &&
                    SphericalGeometry.distanceBetween(next.getLocn(), pt) < shortest){
                nearestpt = next;
                shortest = SphericalGeometry.distanceBetween(next.getLocn(), pt);
            }
        }
        return nearestpt;
    }
    ...
    //HELPER I ADDED
    public void addStopToManager(Stop s){
