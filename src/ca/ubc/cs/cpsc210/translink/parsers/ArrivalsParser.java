...
public class ArrivalsParser {
    ...
    public static void parseArrivals(Stop stop, String jsonResponse)
        ...
        for (int index = 0; index < array.length(); index++) {
            try {
                JSONObject jbusroute = array.getJSONObject(index);
                String routeNo = jbusroute.getString("RouteNo");
                String name = jbusroute.getString("RouteName");
                JSONArray schedules = jbusroute.getJSONArray("Schedules");

                Route r = RouteManager.getInstance().getRouteWithNumber(routeNo);
                r.setName(name);
                parseSchedule(schedules, r, stop);
            }catch (JSONException e){ throw new ArrivalsDataMissingException();}
        }
        if (counter == 0) throw new ArrivalsDataMissingException();
    }

    private static void parseSchedule (JSONArray array, Route r, Stop s) throws JSONException, ArrivalsDataMissingException {

        for (int index = 0; index < array.length(); index++) {
            try{
                JSONObject jarrival = array.getJSONObject(index);
                int expectedCountdown = jarrival.getInt("ExpectedCountdown");
                String destination = jarrival.getString("Destination");
                String ScheduleStatus = jarrival.getString("ScheduleStatus");
                ...
                System.out.println("Some Error occur here");
            }
        }
    }
}
